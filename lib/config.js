...
module.exports = {
    config: options => {
        if (!options.dir.startsWith('/')) {
            // If it doesn't start with a leading `/` then it's relative
            // No need to check for `c:` as device agent is Unix only
            options.dir = fs.join(process.pwd, options.dir)
        }
