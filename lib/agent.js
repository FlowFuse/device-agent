...
class Agent {
    ...
    printAgentStatus (title = null) {
        ...
        if (this.currentOwnerType === 'application') {
            ...
            info('  * Snapshot           : 0 (Application Mode)') // TODO: remove this when we have a better solution for snapshots on devices at application level
        } else {
    ...
    async start () {
        if (this.config?.provisioningMode) {
            ...
            if (this.config?.brokerURL) {
                ...
                this.mqttClient.setProject(this.currentApplication)
    ...
    get currentOwnerType () {
        this._currentOwnerType = (this.currentApplication ? 'application' : (this.currentProject ? 'project' : 'none'))
        return this._currentOwnerType
    }
    ...
    async setState (newState) {
        ...
        // if not, try to determine it from properties
        if (Object.prototype.hasOwnProperty.call(newState, 'ownerType') === false) {
            newState.ownerType = newState.application ? 'application' : (newState.project ? 'project' : null)
        }
        ...
        if (newState === null) {
            ...
            if (Object.prototype.hasOwnProperty.call(newState, 'application')) {
            ...
            if (unknownOrStopped && inhibitUpdates && snapShotUpdatePending && (projectUpdatePending || applicationUpdatePending)) {
                ...
                if (Object.prototype.hasOwnProperty.call(newState, 'application') && (!this.currentSnapshot || newState.application !== this.currentApplication)) {
                    ...
                    updateSnapshot = true
                    updateSettings = true
                } else if (Object.prototype.hasOwnProperty.call(newState, 'project') && (!this.currentSnapshot || newState.project !== this.currentProject)) {
                    info('New instance assigned')
                    this.currentApplication = null
                    this.currentProject = newState.project
                    // Update everything
