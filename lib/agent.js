...
const { compareNodeRedData } = require('./utils.js')
        ...
        if (newState !== null && newState.mode && newState.mode !== this.currentMode) {
            ...
            if (modeChanged) {
                ...
                if (newState.mode === 'developer') {
                    info('Switching on developer mode')
                    ...
                    newState.reloadSnapshot = compareNodeRedData(forgeSnapshot, diskSnapshot) === false
                    if (newState.reloadSnapshot) {
                        info('Current Node-RED files differ from the Platform snapshot, a reload will be requested')
                    } else {
                    ...
                    info('Switching off developer mode')
                }
