...
const OLD_PROJECT_FILE = 'flowforge-project.json'
const PROJECT_FILE = 'flowfuse-instance.json'
...
class Agent {
    constructor (config) {
        ...
        // Handle migration from old 'flowforge' name to 'flowfuse'
        const oldProjectFilePath = path.join(this.config.dir, OLD_PROJECT_FILE)
        if (existsSync(oldProjectFilePath) && !existsSync(this.projectFilePath)) {
            // Log that we're moving from OLD_PROEJCT_FILE to PROJECT_FILE
            info('Migrating legacy project file to new instance file')
            try {
                renameSync(oldProjectFilePath, this.projectFilePath)
                info('Successfully migrated project file')
            } catch (err) {
                warn(`Error migrating project file: ${err.toString()}`)
            }
        }
    }
    ...
    printAgentStatus (title = null) {
        ...
        info(`  * Local Login        : ${this.currentSettings?.security?.localAuth?.enabled ? 'enabled' : 'disabled'}`)
    ...
    async setState (newState) {
        ...
        try {
            ...
            if (newState === null) {
                ...
                // Device has been unassigned from an application
                ...
                // Device has been unassigned from an instance (project)
                ...
                if (unknownOrStopped && developerMode && snapShotUpdatePending && (projectUpdatePending || applicationUpdatePending)) {
                    ...
                    updateSnapshot = true
                    updateSettings = true
                } else if (developerMode === false || forceUpdate === true) {
