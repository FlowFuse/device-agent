    ...
    async checkIn () {
        ...
        if (!state.downloading) {
            const payload = {
                ...
                settings: state.currentSnapshot?.device?.hash || null,
            ...
            this.client.post('live/state', {
                ...
                if (err.response) {
                    if (err.response.statusCode === 409) {
                        ...
                        if (response.snapshot !== state.currentSnapshot) {
                            info('New snapshot available')
                            state.currentSnapshot = await this.getLatestSnapshot()
                            state.downloading = true
                        }
                        if (response.settings !== state.currentSnapshot.device?.hash) {
                            info('New settings available')
                            state.currentSnapshot.device = await this.getSettings()
                            state.downloading = true
                        }
                        
                        if (state.currentSnapshot.id) {
                            info(`New snapshot: ${state.currentSnapshot.id}`)
                            if (state.launcher) {
                                info('Stopping current snapshot')
                                await state.launcher.stop()
                                state.launcher = undefined
                            }
                            await fs.writeFile(this.projectFilePath, JSON.stringify(state.currentSnapshot))
                            state.launcher = Launcher(this.config, state.currentSnapshot)
