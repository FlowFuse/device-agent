...
class MQTTClient {
    ...
    constructor (agent, config) {
        ...
        this.resourcesTopic = `ff/v1/${this.teamId}/d/${this.deviceId}/resources`
        ...
        /** @type {MCP} */
        this.mcp = new MCP({
            nodeRedUserDir: path.join(config.dir, 'project'),
            port: config.port,
            host: config.host
        })
    ...
    start () {
        ...
        this.client.on('error', (err) => {
            ...
            debug(`MQTT connection error: ${err.message}`)
            if (err.cause) {
                debug(`MQTT connection error: ${err.cause.toString()}`)
                debug(`MQTT connection error: ${err.cause.message}`)
            } else if (err.errors && Array.isArray(err.errors)) {
                for (const e in err.errors) {
                    debug(`MQTT connection error: ${err.errors[e].toString()}`)
                }
            }
        })
        ...
        this.client.on('message', async (topic, message, packet) => {
            ...
            try {
                ...
                } else if (msg.command === 'startResources') {
                    if (!this.resourceEnabled) {
                        this.client.publish(this.resourcesTopic, JSON.stringify(this.agent.launcher.sampleBuffer.toArray()))
                    }
                    this.resourceEnabled = true
                } else if (msg.command === 'stopResources') {
                ...
                } else if (msg.command === 'reportPackages') {
                    this.reportPackages = true
                } else if (msg.command === 'get-liveState') {
                    debug('Getting instance live state')
                    try {
                        const response = await this.agent.getState() || {}
                        response.mcp = {
                            supportedCommands: ['mcp:get-features', 'mcp:call-tool', 'mcp:read-resource']
                        }
                        this.sendCommandResponse(msg, response)
                    } catch (error) {
                        warn(`Error getting instance live state: ${error}`)
                        this.sendCommandResponse(msg, { error: error.toString() })
                    }
                } else if (msg.command === 'mcp:get-features') {
                    // MCP ROUTE: step 3 (remote)
                    // Called By: the Forge platform (via MQTT command message)
                    // Calls To : MCP helper class
                    debug('Getting MCP features')
                    try {
                        const { mcpEndPoints } = msg.payload || {}
                        const response = await this.mcp.getFeatures(mcpEndPoints)
                        this.sendCommandResponse(msg, response)
                    } catch (error) {
                        warn(`Error getting MCP features: ${error}`)
                        this.sendCommandResponse(msg, { error: error.toString() })
                    }
                } else if (msg.command === 'mcp:call-tool') {
                    debug('Calling MCP tool', msg.payload)
                    try {
                        const { endpoint, name, input } = msg.payload || {}
                        const response = await this.mcp.callTool(endpoint, name, input)
                        this.sendCommandResponse(msg, response)
                    } catch (error) {
                        warn(`Error calling MCP tool: ${error}`)
                        this.sendCommandResponse(msg, { error: error.toString() })
                    }
                } else if (msg.command === 'mcp:read-resource') {
                    debug('Reading MCP resource', msg.payload)
                    try {
                        const { endpoint, uri } = msg.payload || {}
                        const response = await this.mcp.readResource(endpoint, uri)
                        this.sendCommandResponse(msg, response)
                    } catch (error) {
                        warn(`Error reading MCP resource: ${error}`)
                        this.sendCommandResponse(msg, { error: error.toString() })
                    }
                } else {
    ...
    resource (sample) {
        if (this.resourceEnabled) {
            this.client.publish(this.resourcesTopic, JSON.stringify(sample))
        }
    }
