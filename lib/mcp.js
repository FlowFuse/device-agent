...
class MCP {
     ...
     *
     * The agent opens a fresh session for every operation, so it must also close
     * them: the SDK client's `close()` only aborts the local connection — it does
     * NOT send the HTTP DELETE that terminates the session on the server. Without
     * an explicit `terminateSession()`, server-side sessions accumulate (the MCP
     * host has no idle reaper), and every subsequent connection re-runs its
     * registration pass across all of them. So we terminate, then close.
