...
const RESOURCE_POLL_INTERVAL = 10 // seconds
...
class Launcher {
    constructor (agent, application, project, snapshot, settings, mode) {
        ...
        this.sampleBuffer = new SampleBuffer(2520)
        this.sampleBuffer.setMQTT(agent.mqttClient)
    ...
    async writePackage () {
        ...
        if (packageData.dependencies?.['@flowfuse/nr-table-nodes'] && process.env.NODE_ENV === 'development') {
            ...
            if (existsSync(devPath)) {
                packageData.dependencies['@flowfuse/nr-table-nodes'] = `file:${devPath}`
            }
        }
        // if we are working in a development env and the nr-mqtt-nodes src is available in the right place,
        // then use the development version of the nr-mqtt-nodes
        if (packageData.dependencies?.['@flowfuse/nr-mqtt-nodes'] && process.env.NODE_ENV === 'development') {
            const devPath = path.join(__dirname, '..', '..', 'nr-mqtt-nodes')
            if (existsSync(devPath)) {
                packageData.dependencies['@flowfuse/nr-mqtt-nodes'] = `file:${devPath}`
            }
        }
    ...
    async writeSettings () {
        ...
        if (this.config.brokerURL) {
            const enabled = !!(this.settings?.features ? this.settings.features.teamBroker : true)
            mqttNodes = {
                featureEnabled: enabled,
                token: this.config.token,
                linked: !!this.settings?.mqttNodes?.linked,
                broker: {
                    url: enabled ? this.config.brokerURL || '' : '',
                    username: enabled ? (this.settings?.mqttNodes?.username || `device:${this.config.deviceId}` || '') : '',
                    password: enabled ? (this.settings?.mqttNodes?.password || this.config.brokerPassword || '') : ''
                }
            }
        }

        if (this.settings?.features?.tables) {
            tables = {
                token: this.config.token
            }
        }
        ...
        if (this.settings?.assistant?.mcp && typeof this.settings.assistant.mcp === 'object') {
            assistant.mcp = this.settings.assistant.mcp
        }
        if (this.settings?.assistant?.completions && typeof this.settings.assistant.completions === 'object') {
            assistant.completions = this.settings.assistant.completions
        }
        const settings = {
        ...
        if (this.settings?.editor?.apiMaxLength) {
            settings.apiMaxLength = this.settings.editor.apiMaxLength
        }
        ...
        settings.nodesDir.push(path.join(__dirname, 'plugins', 'node_modules', '@flowfuse', 'flowfuse-resources-plugin'))
        const tablesEnabled = this.settings?.features ? this.settings.features.tables : false
        if (tablesEnabled && this.config.licensed) {
            settings.flowforge.tables = {
                token: this.config.token
            }
        }
        if (this.config.licensed) {
    ...
    async start () {
        ...
        let nodeJSArgs = this.config.nodeOptions
        if (!nodeJSArgs) {
            nodeJSArgs = ['--max-old-space-size=512']
        } else if (!nodeJSArgs.some(opt => opt.startsWith('--max-old-space-size='))) {
            nodeJSArgs.push('--max-old-space-size=512')
        }
        processArgs.unshift(
            ...nodeJSArgs,
            execPathJS
        ...
        this.resourcePoll = setInterval(async () => {
            const pollUrl = `http${this.config.https ? 's' : ''}://127.0.0.1:${this.config.port}/device-editor/ff/metrics`
    ...
    async stop (clean, reason) {
        ...
        const postShutdownOps = async () => {
            ...
            await this.logAuditEvent('stopped', { info: { code: this.stopReason || 'unknown', info: 'Node-RED Stopped' } })
            // short sleep to allow any messages to be sent to FF before we kill the http/mqtt client & actually shutdown/stop the launcher
    ...
    reportPackages () {
        const { modules } = this.readPackage()
        const realList = {}
        const modulesList = Object.keys(modules)
        for (const mod of modulesList) {
            try {
                const modPath = path.join(this.projectDir, 'node_modules', mod, 'package.json')
                const content = fsSync.readFileSync(modPath)
        ...
        return {
            moduleCache: !!this.config.moduleCache,
            packageList: realList
        }
    }
}
