...
class Launcher {
    ...
    async start () {
        ...
        if (this.project.device?.env) {
            Object.assign(env, this.project.device.env)
        }

        info('Starting project')
        const appEnv = env //this.project.env
        const processArgs = [
            '-u',
            this.projectDir
        ]
...
module.exports = {
    Launcher: (config, project) => new Launcher(config, project)
}
