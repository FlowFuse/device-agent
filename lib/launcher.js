...
class Launcher {
    constructor (config, project, snapshot, settings, mode) {
        ...
        this.files = {
            ...
            themeDir: path.join(this.projectDir, 'nodes/@flowforge/nr-theme')
        }
    }
    ...
    async writeThemeFiles () {
        ...
        try {
            const themeSource = path.join(__dirname, '../node_modules/@flowforge/nr-theme')
            if (!existsSync(themeSource)) return
            await fs.mkdir(this.files.themeDir, { recursive: true })
            await fs.cp(themeSource, this.files.themeDir, { recursive: true })
        } catch (error) {
            info(`Could not write theme files to disk: '${this.files.themeDir}'`)
        }
    }
